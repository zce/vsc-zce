...
import { notesToMarkdown } from '../modules/review/markdownExport';
import { applyContentChangesToRange } from '../modules/review/rangeTracking';
import { ReviewNote } from '../modules/review/types';
...
suite('Review location', () => {
	test('formats single line location', () => {
		...
		assert.strictEqual(formatLocation(note), 'src/foo.ts:42');
	});
	...
	test('formats multi-line location', () => {
		...
		assert.strictEqual(formatLocation(note), 'src/foo.ts:42-44');
	});
});

suite('Review Markdown Export', () => {
	test('formats a single note without title or separator', () => {
	...
	test('formats multiple notes with separators only', () => {
		const notes: ReviewNote[] = [
			{
				id: '1',
				file: 'src/a.ts',
	...
	test('returns empty string when there are no notes', () => {
		assert.strictEqual(notesToMarkdown([]), '');
	});
});
